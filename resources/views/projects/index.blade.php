      ...
      @forelse($projects as $project)
        <tr>
          ...
          <td class="border px-4 py-2">
            <a href="{{ route('projects.edit', ['project' => $project]) }}" class="text-blue-500">
              {{ __("Editar") }}
            </a>
            <a href="#" class="text-red-500" onclick="event.preventDefault();
              document.getElementById('delete-project-{{ $project->id }}-form').submit();">
              {{ __("Eliminar") }}
            </a>
            <form id="delete-project-{{ $project->id }}-form" action="{{ route('projects.destroy', ['project' => $project]) }}"
